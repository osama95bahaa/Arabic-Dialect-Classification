...
sentences = [(list(dialect.words(fileid)[i:i+60]),category)
             for category in dialect.categories()
             for fileid in dialect.fileids(category)
             for i in range(0,len(dialect.words(fileid)),60)]
...
word_features = sorted(finder.nbest(bgm.raw_freq,1000))
print(len(word_features))
# print(sorted(finder.ngram_fd.items(), key=lambda t: (-t[1], t[0]))[:4000])
# scored = finder.score_ngrams(bgm.likelihood_ratio)
# print(scored[:15])


all_words = nltk.FreqDist(w for w in dialect.words())
Mcommon = all_words.most_common(2000)
topN = [i[0] for i in Mcommon]

combination = word_features + topN

def sentence_feature(sentence):
    features = {}
    word_sent = set(sentence)
    bigm = list(bigrams(sentence))
    for word in combination:
        features['contains({})'.format(word)] = word in bigm or word in word_sent
    return features
